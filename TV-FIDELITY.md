...
## Header wordmark vector (2026-09-13)

The hand-rendered `sans-serif-condensed` wordmark was replaced by the 2024 YouTube vector outlines, including the `#FF0033` play icon and outlined letter shapes. The source is the 1,469 × 317 vector identified as the June/October 2024 logo on Wikimedia Commons; Google’s YouTube API branding page points clients to YouTube’s brand assets and requires the supplied logo to remain fully visible and unaltered in color.

The accepted Home reference measures the complete mark at `[1605,81][1799,123]`, with the red icon at `[1605,81][1666,123]` and white letters at `[1671,84][1799,121]`. Runtime capture `work/wordmark-exact.png` matches all three boxes exactly. Its thresholded foreground count is 4,271 pixels versus 4,285 in the compressed public reference, a 14-pixel (0.33%) rasterization difference. A 0.775% horizontal correction anchored at the vector’s right edge removes the final one-pixel excess at the letters’ left edge. `work/build-wordmark-exact.log` passed in 41s, both APK outputs were refreshed, and the x86 build installed without an AndroidRuntime crash.

## Final search and playback regression (2026-09-13)

`work/search-wordmark-final.png` verifies the updated vector in Search while preserving the accepted layout: speech orb `[156,66][228,138]`, keyboard in the upper center, recommendation heading at `y=575`, and first result image `[156,634][684,931]`. Search history remains intentionally account-local and therefore differs in text and item count from the public screenshot. The refreshed capture is published as `outputs/search-preview.png`.

Opening the selected Home video reached `PlaybackActivity`, completed SABR buffering and rendered continuous video in `work/player-runtime-refresh-20s.png`. The media session reports `app.smarttube.private.tv` in `PLAYING(3)` at speed 1.0 with no playback error. With focus returned from the recommendation row to the control area, `work/player-controls-up.png` places the title near `y=627`, the seek bar at `y=807`, and recommendation images at `y=895`, within the previously measured reference offsets. Pressing Down focuses the recommendations and Leanback deliberately scrolls that row upward; this is an additional interactive state rather than the accepted reference state. `outputs/player-preview.png` now contains the refreshed control-focused capture.
